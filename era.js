...
function process_command(msg) {
    ...
    switch(cmd) {
        case 'help': { cmd_help(msg); break; }
        case 'banappeal': { cmd_banappeal(msg, args); break; }
        //case 'delete': { cmd_delete(msg, args); break; }
        default: { msg.author.send('Command not found :[ Try ``e!help``'); break; }
    }
}
...
function cmd_help(msg) {
    const embed = new Discord.MessageEmbed()
        ...
        .setColor('#fc8c03').setTitle('Commands').setThumbnail('https://i.imgur.com/vrRImoI.png')
        .addFields(
	    	{ name: 'e!help', value: 'displays all of our commands' },
            { name: 'e!banappeal', value: 'appeal a ban for our admins to discuss it' },
	    )
...
function cmd_banappeal(msg, args) {
    ...
    if (args.length < 3) {
        const embed = new Discord.MessageEmbed()
            ...
            .setTitle(':information_source:  Ban appeal usage  :information_source:').setDescription('```e!banappeal <name> <link> <reason>```')
            .addFields(
	            { name: 'name', value: 'your ingame nickname', inline: true },
                { name: 'link', value: 'your current steam account link', inline: true },
                { name: 'reason', value: 'reason on why we should unban you', inline: true },
            )
            .setFooter('USING THIS COMMAND IN A NON SERIOUS WAY WILL RESULT IN A BAN', 'https://i.imgur.com/cj2KuzF.png')
    ...
    const embed = new Discord.MessageEmbed()
	    .setColor('#0099ff').setTitle(':oncoming_police_car: NEW BAN APPEAL :oncoming_police_car:').setTimestamp()
	    .setAuthor('ERA DISCORD BOT', 'https://i.imgur.com/hOuIomW.png', 'https://steamcommunity.com/groups/EraSurfCommunity')
	    .setDescription('Lets decide if we keep this player banned or not.').setThumbnail('https://i.imgur.com/vrRImoI.png')
	    .addFields(
    ...
    setTimeout( () => { 
        ...
        setTimeout(() => {
            const channel = server.channels.cache.find(guild => guild.name === 'banappeal-'+playername);
            channel.send(embed);
