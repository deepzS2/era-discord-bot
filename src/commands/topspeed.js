...
const aliases = [
  [ '#2c5b9c', 'https://i.imgur.com/5IjozjO.png', 'surf_greatriver_xdre4m_era', 'xdr', 'xdream', 'xdre4m', 'greatriver_xdream', 'greatriver_xdre4m', ],
  [ '#9dc4fa', 'https://i.imgur.com/9dElAnJ.png', 'surf_greatriver_evil_v2', 'evil', 'greatriver_evil'],
  [ '#d19c38', 'https://i.imgur.com/Gg9xStx.png', 'surf_ski_2_inf_v1e', 'ski_2_go', 'ski_2', 'ski2'],
  ...
  [ '#8a7f63', 'https://i.imgur.com/yEFWM9I.png', 'surf_home_sweet_home_v4', 'hsh', 'homesweethome', 'home-sweet-home', 'home_sweet_home'],
  [ '#c7bf93', 'https://i.imgur.com/kQ1Dfij.jpg', 'surf_skyscraper_v5', 'skyscraper'],
  [ '#f5cb71', 'https://i.imgur.com/6omdrnb.jpg', 'surf_japan_ptad_v5', 'japan'],
  [ '#f5b771', 'https://i.imgur.com/BFOO26U.jpg', 'surf_ny_bigloop_nf_e', 'bigloop', 'big_loop', 'big-loop'],
  [ '#ebd4a9', 'https://i.imgur.com/C9yTfaQ.png', 'surf_buzzkill2_zavv', 'buzz', 'buzkill', 'buzzkill2', 'buzz2'],
  [ '#d6b26f', 'https://i.imgur.com/qvO4yBI.png', 'surf_dust2_2015', 'dust', 'dust2', 'dust2015', 'dust2_2015', 'dust_2015'],
  [ '#9c6424', 'https://i.imgur.com/sYpC9qk.png', 'surf_skyworld_nf_v2e', 'skyworld'],
  [ '#ffd500',  'google.com', 'surf_monster_dust2_era', 'monster', 'monster_dust2', 'monster_dust_2', 'monsterdust2'],
  ...
  [ '#ffbb00', 'https://i.imgur.com/9oKakT7.jpg',  'surf_10x_reloaded_nf_v2', '10x', '10x_reloaded', '10xreloaded'],
]

const COLOR_INDEX = 0, PICTURE_INDEX = 1, MAPNAME_INDEX = 2;
...
module.exports = {
  ...
  args: [ 
    { name: "`<map>`", value: "*map that you wanna know the top speed of*", inline: false },
    { name: "`[count]`", value: "*how many players? (1 = shows only TOP 1)*", inline: false, },
  ],
  usage: "e!topspeed <map> [count]",
  cooldown: 10,
  disabled: false,
  //footer: "IN CASE YOU DONT KNOW MAP NAMES, USE e!maplist OR e!mappool", // todo
  execute(message, args) {
    if (args.length < 1 || args.length > 2) {
      return message.reply('❌  **wrong usage**, please check **`e!help topspeed`**  😾');
    }
    ...
    if (!isNaN(map)) {
      return message.reply('❌  **error! `<map>`** argument needs to be map name, please check **``e!help topspeed``**  ❌');
    }

    if (args.length == 2) {
      count = args[1];
      if (isNaN(count)) {
        return message.reply('❌  **error! `<count>`** argument needs to be number, please check **``e!help topspeed``**  ❌');
      } else if (count > 10) {
        return message.reply('❌  **error! `<count>`** argument needs to be less than 10  ❌');
      }
    }
    ...
    connection.query(query_topspeed, function (error, results, fields) {
        ...
        if (results.length) {
          const topspeedmap_embed = new MessageEmbed()
	          .setColor(get_map_color(map)).setTitle('Top speed on `'+map+'`  🏄‍♂️').setImage(get_map_picture(map))
	          //.setAuthor("ERA DISCORD BOT", "https://i.imgur.com/vrRImoI.png", "https://steamcommunity.com/groups/EraSurfCommunity");
            
            const medals = [ ' 🥇', ' 🥈', ' 🥉' , ' 🎖️'];
            let i = 1;
            results.forEach((player) => {
              topspeedmap_embed.addField(medals[i >= 4 ? 3:i-1] + ' ' + i++ + '. ' + player.name, player.speed + ' u/s');
            })
